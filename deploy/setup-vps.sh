...
#
#  前置�?#    1. 域名 A 记录已指 VPS 公网 IP
#    2. 本地�?`ssh-copy-id -i studio.pem ubuntu@<VPS_IP>`
#    3. 已从 GitHub 拉取本仓库（手动�?clone�?# ============================================================
...
REPO_DIR="$APP_DIR/repo"                # git clone 的源�?BUILD_DIR="$APP_DIR/build"              # 每次 build 的输�?PUBLIC_DIR="$APP_DIR/public"            # Caddy 服务的根（rsync 同步�?BUILD_DIR�?CADDYFILE="/etc/caddy/Caddyfile"
REPO_URL="https://github.com/Luo-root/jiangnan-blog.git"
BRANCH="main"
LOG_DIR="/var/log/blog-deploy"
WORKBENCH_DIR="/home/studio/workbench"  # 后续�?Obsidian 工作台数据源
...
if [ -f /tmp/workbench.tar.gz ]; then
    ...
    echo "    工作台已�?/tmp/workbench.tar.gz 解压�?$WORKBENCH_DIR"
fi

# ---- 4. 克隆代码（优�?GitHub，失�?fallback 本地 tarball�?----
echo "[4/6] 准备代码..."
if [ -d "$REPO_DIR/.git" ]; then
    cd "$REPO_DIR"
    sudo -u "$DEPLOY_USER" git pull --ff-only origin "$BRANCH" 2>&1 || true
elif [ -f /tmp/blog.tar.gz ]; then
    echo "    本地 tarball 解压�?$REPO_DIR"
    mkdir -p "$REPO_DIR"
    tar -xzf /tmp/blog.tar.gz -C "$REPO_DIR"
    cd "$REPO_DIR"
    # �?tarball 时强制重新构建（不依�?lockfile 一致性）
    ...
    echo "ERROR: 既无 /tmp/blog.tar.gz 也连不上 GitHub，请�?scp 上传 tarball"
    exit 1
fi
...
# �?Caddyfile（与 deploy/caddyfile 同步�?cat > "$CADDYFILE" <<EOF
# 遇见江楠 · 静态博�?# 自动 HTTPS (Let's Encrypt)
# www �?主域 301
# 主域 �?静态文�?
...
if [ -d "$BUILD_DIR/dist" ]; then
    ...
    echo "    �?build 失败�?BUILD_DIR/dist 不存�?
    exit 1
fi

# ---- systemd 定时更新（可选，cron 风格�?----
cat > /etc/cron.d/blog-pull <<EOF
# �?5 分钟检�?GitHub，有�?push 自动 build + 部署
...
echo
echo "  Caddy 已自动申�?HTTPS 证书（前提：域名 A 记录已指本机�?
