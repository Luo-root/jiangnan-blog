...
- MCP 实现以 `SCHEMA.md` + `docs/agent-workbase-mcp-v0.1.md` + `server/mcp/README.md` 为准。骨架、索引读路径、写路径 3-way / apply 分阶 / 敏感 warning 已按契约落地；热度 `Hot()` 公式与审计 SQLite 仍是后续 PR。不要把旧 `noteID()` / payload-as-ours / 默认敏感正则当规格。
